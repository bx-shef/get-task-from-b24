...
✅ **Поле годится для поиска — замерено 2026-08-27** целью `TASK=<id> make uf-probe`:
`tasks.task.list` с фильтром объектом находит ровно нужную задачу; формы фильтра массивом
этот метод отвергает. На этом строится замена журнала переносов
([`PRODUCT.md`](PRODUCT.md), раздел 1а).
