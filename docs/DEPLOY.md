...
⚠ **Годится ли поле для поиска — открытый вопрос.** Прежнее «не годится» снято: замер
слал фильтр в форме старого API. Перемер — `TASK=<id> make uf-probe`. Сверка в любом
случае идёт по журналу `transfers` (`make transfers`): он не зависит от портала.
