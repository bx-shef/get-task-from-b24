...
export const MIGRATIONS: readonly { id: string; sql: string }[] = [
  ...
  },
  {
    ...
    // Индекс журнала переносов. Шаг остался в списке ровно потому, что он уже отработал
    // на боевой базе: применённые шаги не меняют и не удаляют (см. выше). Сам журнал
    // снят шагом 003.
    sql: `create index if not exists transfers_updated_at_idx on transfers (updated_at desc);`,
  },
  {
    id: '003-drop-transfers',
    /**
     * Журнал переносов снят: связка «задача клиента → наша задача» живёт в UF-полях
     * самой задачи (docs/PRODUCT.md, раздел 1а).
     *
     * ⚠ Это УДАЛЕНИЕ БОЕВЫХ ДАННЫХ, и происходит оно автоматически при первом старте
     * нового образа. История переносов из таблицы исчезает; сами переносы никуда не
     * деваются — они и есть задачи на нашем портале. Кому нужна история — снимать
     * `make backup` ДО выката (docs/DEPLOY.md).
     */
    sql: `drop table if exists transfers;`,
  },
]
