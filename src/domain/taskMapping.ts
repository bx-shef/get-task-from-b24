...
export function buildTargetTask(source: SourceTaskFull, options: BuildOptions): TargetTaskFields {
  ...
  //
  // ⚠ По этому полю задача ИЩЕТСЯ, и это замер (2026-08-27, боевой портал, цель
  // `make uf-probe`): `tasks.task.list` с фильтром объектом `filter[UF_SOURCE_TASK_ID]`
  // возвращает ровно нужную задачу. Формы фильтра массивом метод отвергает с 400.
  // Значение приходит строкой, хотя поле числовое.
  //
  // ⚠ На этом строится замена журнала переносов: «переносилась ли задача» спрашивается
  // у портала по паре с `UF_SOURCE_DOMAIN`. Поэтому оба поля обязаны попадать в задачу
  // всегда — без них перенос теряет свой единственный ключ.
