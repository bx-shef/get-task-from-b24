...
export function buildTargetTask(source: SourceTaskFull, options: BuildOptions): TargetTaskFields {
  ...
  //
  // ⚠ По этому полю задача ИЩЕТСЯ: `tasks.task.list` с фильтром объектом возвращает
  // ровно нужную. Замер и отвергнутые формы фильтра — в `docs/PRODUCT.md`, раздел 1;
  // значение оттуда приходит строкой, хотя поле числовое.
  //
