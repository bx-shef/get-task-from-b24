...
export function buildTargetTask(source: SourceTaskFull, options: BuildOptions): TargetTaskFields {
  ...
  //
  // ⚠ Ищется ли задача по этому полю — не установлено. Прежний замер («не ищется»)
  // слал фильтр объектом, то есть в форме старого API; REST v3 ждёт массив условий.
  // Перемер — цель `make uf-probe`. Независимо от ответа сопоставление переносов
  // держится на журнале (`transfers`), а не на портале: журнал не зависит от того,
  // отвечает ли портал.
